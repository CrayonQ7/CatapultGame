...
bool GameRuleLayer::init(bool isBegin)
{
	...
	else
	{
		...
		backItem->setPosition(Vec2(origin.x + visibleSize.width / 2, origin.y + backItem->getContentSize().height));
		auto menu = Menu::create(backItem, NULL);
		menu->setPosition(Vec2::ZERO);
		addChild(menu, 1);
	}
	// 标题
	auto label = Label::createWithTTF("Description", "fonts/Marker Felt.ttf", 40);
